...
h36m_skeleton = Skeleton(parents=[-1, 0, 1, 2, 3, 4, 0, 6, 7, 8, 9, 0, 11, 12, 13, 14, 12,
                         ...
                         joints_right=[1, 2, 3, 4, 5, 24, 25, 26, 27, 28, 29, 30, 31])   # 原始的skeleton索引关系

h36m_skeleton_joints_group = [[2, 3], [5, 6], [1, 4], [0, 7], [8, 9], [14, 15], [11, 12], [10, 13]]   # 新的索引关系
...
H36M_NAMES[3] = 'RFoot'       # 3  RAnkle
H36M_NAMES[6] = 'LHip'        # 4
H36M_NAMES[7] = 'LKnee'       # 5
H36M_NAMES[8] = 'LFoot'       # 6  LAnkle
H36M_NAMES[12] = 'Spine'      # 7
H36M_NAMES[13] = 'Thorax'     # 8
H36M_NAMES[14] = 'Neck/Nose'  # Don't predict this joint
...
# Human3.6M for training and testing
...
class Human36mDataset(MocapDataset):
    def __init__(self, path, remove_static_joints=True):
        ...
        self._cameras = copy.deepcopy(h36m_cameras_extrinsic_params)  # 'orientation'和'translation'
        for cameras in self._cameras.values():   # 10个subject，每个subject有4个cameras
            for i, cam in enumerate(cameras):  # 对每一个camera
                cam.update(h36m_cameras_intrinsic_params[i])  # 把相机内参字典的键/值对更新到cam里。
        ...
        data = np.load(path, allow_pickle=True, encoding='latin1')['positions_3d'].item()  # 字典  {'S9': {'Phoning 1': array([[[-0.0297206 , -0.303175  ,  0.990323  ],
                                                                                               #         [-0.15256992, -0.2987592 ,  1.0071473 ],
                                                                                               #         [-0.16644731, -0.26083943,  0.53668386],

        self._data = {}
        for subject, actions in data.items():   # .items(), Yield (key, value) pairs
            self._data[subject] = {}
            for action_name, positions in actions.items():
                self._data[subject][action_name] = {   # .keys(), 增加了一个cameras的键, dict_keys(['positions', 'cameras'])
                    'positions': positions,
